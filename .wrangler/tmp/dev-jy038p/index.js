...
// .wrangler/tmp/bundle-PxcIEl/strip-cf-connecting-ip-header.js
...
__name(wordRowToRecord, "wordRowToRecord");
async function getAllWords(db) {
  const result = await db.prepare("SELECT * FROM words ORDER BY created_at DESC").all();
  return result.results.map(wordRowToRecord);
}
__name(getAllWords, "getAllWords");
async function createWord(db, input) {
...
__name(createWord, "createWord");
async function updateWord(db, id, input) {
  const updates = [];
  const values = [];
  if (input.correct !== void 0) {
    updates.push("correct = ?");
    values.push(input.correct);
  }
  if (input.miss !== void 0) {
    updates.push("miss = ?");
    values.push(input.miss);
  }
  if (input.lastPlayed !== void 0) {
...
__name(updateWord, "updateWord");
async function deleteWord(db, id) {
  await db.prepare("DELETE FROM words WHERE id = ?").bind(id).run();
}
__name(deleteWord, "deleteWord");
async function deleteAllWords(db) {
  await db.prepare("DELETE FROM words").run();
}
__name(deleteAllWords, "deleteAllWords");
async function bulkInsertWords(db, words, clearExisting = false) {
  if (clearExisting) {
...
__name(bulkInsertWords, "bulkInsertWords");
async function getAggregatedStats(db) {
  const result = await db.prepare("SELECT * FROM aggregated_stats WHERE id = 1").first();
  if (!result) {
    return null;
  }
  return {
...
__name(getAggregatedStats, "getAggregatedStats");
async function upsertAggregatedStats(db, input) {
  await db.prepare(
...
__name(upsertAggregatedStats, "upsertAggregatedStats");
async function deleteAggregatedStats(db) {
  await db.prepare("DELETE FROM aggregated_stats WHERE id = 1").run();
}
__name(deleteAggregatedStats, "deleteAggregatedStats");
...
async function upsertSettings(db, input) {
  ...
  if (existing) {
    ...
    await db.prepare(
      ...
      input.srsEnabled !== void 0 ? input.srsEnabled ? 1 : 0 : 1,
      input.warmupEnabled !== void 0 ? input.warmupEnabled ? 1 : 0 : 1,
...
async function getAllGameScores(db) {
  const result = await db.prepare("SELECT * FROM game_scores ORDER BY played_at DESC").all();
...
__name(getAllGameScores, "getAllGameScores");
async function createGameScore(db, input) {
  const result = await db.prepare(
...
__name(createGameScore, "createGameScore");
async function deleteAllGameScores(db) {
  await db.prepare("DELETE FROM game_scores").run();
}
__name(deleteAllGameScores, "deleteAllGameScores");
...
// .wrangler/tmp/bundle-PxcIEl/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-PxcIEl/middleware-loader.entry.ts
